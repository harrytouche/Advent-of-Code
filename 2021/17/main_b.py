...
x_min, x_max, y_min, y_max = [
    int(x)
    for x in re.match("target area: x=(.+)\.\.(.+), y=(.+)\.\.(.+)", input_data)[1:]
]
...
def is_in_target(d):
    ...
    return (
        True
        if (min(x_min, x_max) <= x <= max(x_min, x_max))
        and (min(y_min, y_max) <= y <= max(y_min, y_max))
        else False
    )
...
def is_past_target(d):
    x, y = d
    return True if (x > max(x_min, x_max)) or (y < min(y_min, y_max)) else False
